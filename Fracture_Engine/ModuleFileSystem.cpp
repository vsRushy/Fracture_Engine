...
ModuleFileSystem::ModuleFileSystem(const char* game_path) : Module()
{
	...
	// Dump list of paths
	LOG(LOG_INFORMATION, "FileSystem Operations base is [%s] plus:", GetBasePath());
	LOG(LOG_INFORMATION, GetReadPaths());

	// enable us to write in the game's dir area
	if (PHYSFS_setWriteDir(".") == 0)
		LOG(LOG_ERROR, "File System error while creating write dir: %s\n", PHYSFS_getLastError());
...
{
	LOG(LOG_INFORMATION, "Loading File System");
	bool ret = true;
...
{
	LOG(LOG_INFORMATION, "Freeing File System subsystem");
...
bool ModuleFileSystem::AddPath(const char* path_or_zip)
{
	...
	{
		LOG(LOG_ERROR, "File System error while adding a path or zip: %s\n", PHYSFS_getLastError());
	}
	else
		ret = true;
...
bool ModuleFileSystem::CopyFromOutsideFS(const char* full_path, const char* destination)
{
	...
	if (source && dest)
	{
		...
		LOG(LOG_INFORMATION, "File System copied file [%s] to [%s]", full_path, destination);
	}
	else
		LOG(LOG_ERROR, "File System error while copy from [%s] to [%s]", full_path, destination);
...
bool ModuleFileSystem::Copy(const char* source, const char* destination)
{
	...
	if (src && dst)
	{
		...
		LOG(LOG_INFORMATION, "File System copied file [%s] to [%s]", source, destination);
	}
	else
		LOG(LOG_ERROR, "File System error while copy from [%s] to [%s]", source, destination);
	...
	if (fs_file != nullptr)
	{
		...
		if (size > 0)
		{
			*buffer = new char[size];
			...
			if (readed != size)
			{
				LOG(LOG_ERROR, "File System error while reading from file %s: %s\n", file, PHYSFS_getLastError());
				delete buffer;
			}
			else
				ret = readed;
		...
		if (PHYSFS_close(fs_file) == 0)
			LOG(LOG_ERROR, "File System error while closing file %s: %s\n", file, PHYSFS_getLastError());
	}
	else
		LOG(LOG_ERROR, "File System error while opening file %s: %s\n", file, PHYSFS_getLastError());
...
uint ModuleFileSystem::Save(const char* file, const void* buffer, unsigned int size, bool append) const
{
	...
	if (fs_file != nullptr)
	{
		...
		if (written != size)
		{
			LOG(LOG_ERROR, "File System error while writing to file %s: %s", file, PHYSFS_getLastError());
		}
		else
		{
			if (append == true)
			{
				LOG(LOG_INFORMATION, "Added %u data to [%s%s]", size, PHYSFS_getWriteDir(), file);
			}
			//else if(overwrite == true)
				//LOG("File [%s%s] overwritten with %u bytes", PHYSFS_getWriteDir(), file, size);
			else if (overwrite == false)
			{
				LOG(LOG_INFORMATION, "New file created [%s%s] of %u bytes", PHYSFS_getWriteDir(), file, size);
			}
		...
		if (PHYSFS_close(fs_file) == 0)
			LOG(LOG_ERROR, "File System error while closing file %s: %s", file, PHYSFS_getLastError());
	}
	else
		LOG(LOG_ERROR, "File System error while opening file %s: %s", file, PHYSFS_getLastError());
...
bool ModuleFileSystem::Remove(const char* file)
{
	...
	if (file != nullptr)
	{
		...
		{
			LOG(LOG_INFORMATION, "File deleted: [%s]", file);
			ret = true;
		}
		else
			LOG(LOG_ERROR, "File System error while trying to delete [%s]: ", file, PHYSFS_getLastError());
	}
...
size_t AssimpWrite(aiFile* file, const char* data, size_t size, size_t chunks)
{
	...
	if (ret == -1)
		LOG(LOG_ERROR, "File System error while WRITE via assimp: %s", PHYSFS_getLastError());
...
size_t AssimpRead(aiFile* file, char* data, size_t size, size_t chunks)
{
	...
	if (ret == -1)
		LOG(LOG_ERROR, "File System error while READ via assimp: %s", PHYSFS_getLastError());
...
size_t AssimpTell(aiFile* file)
{
	...
	if (ret == -1)
		LOG(LOG_ERROR, "File System error while TELL via assimp: %s", PHYSFS_getLastError());
...
size_t AssimpSize(aiFile* file)
{
	...
	if (ret == -1)
		LOG(LOG_ERROR, "File System error while SIZE via assimp: %s", PHYSFS_getLastError());
...
void AssimpFlush(aiFile* file)
{
	if (PHYSFS_flush((PHYSFS_File*)file->UserData) == 0)
		LOG(LOG_ERROR, "File System error while FLUSH via assimp: %s", PHYSFS_getLastError());
}
...
aiReturn AssimpSeek(aiFile* file, size_t pos, aiOrigin from)
{
	...
	if (res == 0)
		LOG(LOG_ERROR, "File System error while SEEK via assimp: %s", PHYSFS_getLastError());
...
void AssimpClose(aiFileIO* io, aiFile* file)
{
	if (PHYSFS_close((PHYSFS_File*)file->UserData) == 0)
		LOG(LOG_ERROR, "File System error while CLOSE via assimp: %s", PHYSFS_getLastError());
}
